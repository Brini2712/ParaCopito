...
// Reemplazar el evento mouseover por una función que detecte si es móvil
function isMobileDevice() {
  return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
}

// Eliminar el evento mouseover anterior y agregar la nueva lógica
btnNo.removeEventListener('mouseover', () => {});

// Nueva función para mover el botón
function moveButton(e) {
  // Si es dispositivo móvil, solo mover en click
  if (isMobileDevice() && e.type === 'mouseover') {
    return;
  }
  ...
  // Obtener dimensiones de la pantalla seguras
  const maxWidth = window.innerWidth - buttonWidth;
  const maxHeight = window.innerHeight - buttonHeight;
  
  // Calcular nueva posición
  const newX = Math.min(Math.max(0, Math.random() * maxWidth), maxWidth);
  const newY = Math.min(Math.max(0, Math.random() * maxHeight), maxHeight);
  
  btnNo.style.position = 'absolute';
  btnNo.style.left = newX + 'px';
  btnNo.style.top = newY + 'px';
  
  currentMessage = btnNo.textContent;
  clickSound.play();
}
